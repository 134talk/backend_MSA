...
public class ChatRoomService {
	...
	public List<ChatroomListDto> findChatRoomsByName(long userId, String teamCode, String name){
		...
		List<Chatroom> chatroomEntity = chatroomRepository.findByTeamCodeAndName(teamCode, findUserIds);
	...
	public List<ChatroomListDto> convertChatRoomListDto(long userId, List<Chatroom> chatroomEntity){
		return chatroomEntity.stream().map(chatroom -> {
			...
			List<Emoticons> emoticons = sizeByCode.entrySet().stream()
					...
					.limit(3)
					.map(entry -> {
						return Emoticons.builder().emoticonCode(entry.getKey()).emoticonCount(entry.getValue())
								.build();
					}).collect(Collectors.toList());

			List<ChatroomUsers> chatroomUsers = chatroom.getChatroomUsers();
			List<Long> userIds = chatroomUsers.stream().map(ChatroomUsers::getUserId).collect(Collectors.toList());
	...
	public void createChatroom(String teamCode, List<Long> userList) {
		Chatroom chatroom = Chatroom.builder().name("").teamCode(teamCode).build();

		List<ChatroomUsers> chatroomUsers = userList.stream().map(userId -> {
			return ChatroomUsers.builder().chatroom(chatroom).userId(userId).build();
		}).collect(Collectors.toList());
